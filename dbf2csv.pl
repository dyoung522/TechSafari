...
#  =>keyflag<=-- "%v, %d"
$ver_ordinal_and_date = "26, 19-Jul-05";
...
# characters) by Jacky Bruno <jacky.bruno@free.fr> collège de Villeneuve
...
#   http://www.e-bachmann.dk/computing/databases/xbase/dbf.html or http://www.e-bachmann.dk/docs/xbase.htm
...
# lines 864 to the end : main program :
...
# lines 745 to 776 : do_a_file function 'the name is clear) :
...
# lines 218 to 740 : cvt1file function :
   ...
   if (($dbf_file ne $current_dbf_file)) {
      ...
      if ($current_memo_file =~ /\.dbt$/) {
         print "ERROR: This program doesn't yet understand .DBT (memo) files.  If you can\n" .
               ...
               "Email me at Dave\@burtonsys.com.\n";
         if ($current_memo_file_is_open) {
            close MEMOFILE;
            $current_memo_file_is_open = 0;
         }
...
sub cvt1file {
   ...
   for ($i=1; $i <= $numfields; $i++) {
      ...
      if ((('I' eq $fldType) || ('+' eq $fldType) || ('M' eq $fldType)) && (4 == $fldLen)) {
         ...
         $recfmt .= "Vx4";  # or maybe "x4V" or "x4N" -- please email dave@burtonsys.com if you figure it out!
         ...
         if (('Y' eq $fldType) || ('T' eq $fldType)) {  # don't know how these are represented
            ...
            if (! defined $fld_typ_seen{$fldType}) {
               print ", which might not\n" .
                  "convert properly (please email Dave\@burtonsys.com \& tell him whether it works).";
             }
            ...
            if (! defined $fld_typ_seen{$fldType}) {
               print ", which is unsupported.\n" .
                  ...
                  "test support for '$fldType' fields, then please email Dave\@burtonsys.com.";
            }
         }
         print "\n";
      }
