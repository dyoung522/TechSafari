...
my %ftphash = (
    ...
    'PASS'   => q(PASSWORD),
    'RDIR'   => q(/CMS/manual/outbound/2646),
);
...
sub mydie {
    my @message = @_;
    mylog( @message );
    send_mail( $problem_email, 'Problem with Daily Move Signal Trigger Update', @message );
    myexit;
}
...
# clean up and exit
if ( $processed ) {
    my $message = 'Daily move signals trigger processing sucessfully completed on ' . scalar localtime(time());
    send_mail( $notify_email, 'Daily MoveSignal Triggers OK', $message );
}
