...
my $ftp_username   = q(tutrig);
my $ftp_password   = q(PASSWORD);
#my $inq_basename   = q(prm.edtout.dgimrkt.inqtrig);
my $inq_basename   = q(PRM.EDTOUT.DGIMRKT.INQTRIG);
...
# Getting file list
foreach my $NAME ( $ftp_connection->ls($inq_basename . '.*.pgp') ) {
...
my $prm_boilerplate = $inq_basename . q{.D%02d%02d%02d.new};
...
# Process each file
foreach $pgp_file_name ( sort qx(ls $inq_basename.* 2>/dev/null) ) {
    chomp $pgp_file_name;
